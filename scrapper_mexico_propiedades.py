...
if __name__ == '__main__':
    ...
    # Construir URL: https://propiedades.com/{ciudad}/{tipo}-{concepto}
    target_url = f"https://propiedades.com/{ciudad}/{tipo}-{concepto}"
    ...
    if not args.output:
        if args.pagina > 1:
            output_file = f"inmueble-propiedades-{concepto}-{tipo}-{ciudad}-{args.pagina}.json"
        else:
            output_file = f"inmueble-propiedades-{concepto}-{tipo}-{ciudad}-1.json"
    else:
